...
import { asNumber, asString, isExpired, ownerFirstName, payloadOf } from '../utils/contextHelpers.js';
const MAX_ACTIONS_PER_TICK = 20;
export class DecisionService {
    store;
    conversations;
    constructor(store, conversations) {
        this.store = store;
    ...
    async handleTick(now, availableTriggers) {
        ...
        const actions = [];
        const merchantsUsedThisTick = new Set();
        for (const trigger of triggerRecords) {
            ...
            this.conversations.recordOutbound(conversationId, merchantId, customerId, trigger.context_id, action.body, suppressionKey);
            merchantsUsedThisTick.add(merchantId);
            actions.push(action);
        }
        return { actions };
    }
    async handleReply(conversationId, merchantId, customerId, fromRole, message) {
    ...
    toTickAction(composed, triggerId, merchantId, customerId, conversationId) {
        return {
    ...
    buildConversationId(triggerId, merchantId, customerId) {
        if (customerId) {
    ...
    isOptOut(message) {
        return (message.includes('stop messaging') ||
    ...
    isCommitment(message) {
        return (message.includes("let's do it") ||
